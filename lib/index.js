...
const { getPortByPid, findRule, kill } = require('./utils');
...
const run = async (rule, cwd) => {
  ...
  // Trying to prevent port conflicts.
  await sleep.randomResolve(0, 500);
  ...
  try {
    rule.proc = proc;
    rule.port = rule.port || await getPortByPid(proc.pid);
    return rule;
  } catch (err) {
    throw err;
  }
...
const createRequestListner = (rules, options, proxy) => {
  return (req, res) => {
...
module.exports = options => {
  ...
  const listener = createRequestListner(rules, options, proxy);
  ...
  for (const rule of rules) {
    ...
    if (rule.run) {
      run(rule, options.cwd)
        .then(rule => {
          console.log(`   Rule for ${rule.pathname} successfully started on ${rule.port} port`);
        })
        .catch(err => {
          console.error(`Rule for ${rule.pathname} does not started successfully`);
          console.error(err);
        });
